...
//
// feed / reveal default to no-op stubs so a base class can stay protocol-
// only and let a derived class layer the input/output machinery on top.
class Backend {
public:
	int party;
	...
	// active backend matches the static wire type.
	virtual size_t wire_bytes() const = 0;

	// Gate dispatch — out-param style so wide wires don't pay
	// return-by-value costs.
	...
	// Bulk variants — n contiguous wires laid out as a flat byte array
	// of n * wire_bytes() with no padding. Default loops the scalar
	// path; backends with a faster shape (e.g. SIMD-vectorizable XOR
	// over block wires) can override.
