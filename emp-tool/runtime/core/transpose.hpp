#ifndef EMP_UTIL_TRANSPOSE_HPP__
#define EMP_UTIL_TRANSPOSE_HPP__

// Bit-matrix transpose kernels for block.h's API. Included via block.h (after
// the block type and block.hpp's basics), exactly like block.hpp itself.
//
// Two entry points:
//   sse_trans(out, inp, nrows, ncols) -- the generic byte-oriented transpose
//     (any nrows%8==0, ncols%8==0).
//   sse_trans_n128(out, inp, ncols)   -- the nrows=128 hot path every OT
//     extension drives; dispatches at compile time on the widest available
...
#endif  // EMP_UTIL_TRANSPOSE_HPP__
