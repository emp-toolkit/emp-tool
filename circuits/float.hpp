...
inline Float::Float(int value_length, int expnt_length, double input, EmpParty party) {
...
template<>
inline string Float::reveal<string>(EmpParty party) const {
    double val = static_cast<double>(value.reveal<long long>(party));
    double exp = static_cast<double>(expnt.reveal<long long>(party));
    return std::to_string(val*pow(2.0, exp));
}

template<>
inline double Float::reveal<double>(EmpParty party) const {
    double val = static_cast<double>(value.reveal<long long>(party));
    double exp = static_cast<double>(expnt.reveal<long long>(party));
    return val*pow(2.0, exp);
}

inline string Float::detail(EmpParty party) const {
...
inline Integer divide_frac(const Integer& lhs, const Integer& rhs) {
    ...
    i1 = i1 << rhs.size();
    Integer res(2 * rhs.size(), 0, PUBLIC);
